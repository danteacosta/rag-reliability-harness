"""RAG-local lifecycle events; neutral delivery contracts remain in ARP."""
...
class LifecycleEvent:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "LifecycleEvent":
        return cls(
            ...
            str(value.get("schema_version", "rag-reliability-events/v1")),
        )
    ...
    def emit(
        ...
        *,
        ...
        event = LifecycleEvent(
            ...
            dict(data or {}),
            str(uuid4()),
            parent_event_id or self._last_event_id,
        )
...
def collect_lifecycle_events(path: Path | str, *, run_id: str) -> list[LifecycleEvent]:
    return [
        ...
        if line.strip()
        for event in (LifecycleEvent.from_dict(json.loads(line)),)

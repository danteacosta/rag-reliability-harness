...
def summarize_utility(observations: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not runs:
        return {
            ...
            "miss_rate": 0.0,
            "captured_regressions": 0,
            "escaped_incidents": 0,
            "cost_usd_total": 0.0,
            "cost_usd_per_captured_regression": None,
    ...
    captured = sum(bool(row.get("alerted")) and bool(row.get("true_incident")) for row in rows)
    costs = sum(float(row.get("cost_usd", 0.0) or 0.0) for row in rows)
    ...
    return {
        ...
        "escaped_incidents": misses,
        "cost_usd_total": costs,
        "cost_usd_per_captured_regression": costs / captured if captured else None,
